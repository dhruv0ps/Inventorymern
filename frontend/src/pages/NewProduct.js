...
const NewProduct = () => {
    ...
    const [variants, setVariants] = useState([{  height: '',
        width: '',
        length: '', firmness: '', price: '', weight: '', color: '', childName: '', rawMaterials: [{ material: '', quantity: '', unit: '' }], tags: [] }]);
...
const handleRemoveTag = (index, tagToRemove) => {
    ...
    const handleCategoryChange = (selectedOptions) => {
        ...
        console.log("Selected Categories:", selectedOptions); // Log to check
    };
    const resetForm = () => {
        setParentName('');
    ...
    return (
        <div className="container w-full bg-navbar ml-10 mr-10  mx-auto mt-5 px-4 max-h-[80vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4 ">
                ...
                <Select
                    id="category-select"
                    options={categories} // Use categories as the options for react-select
                    isMulti // Enable multi-select functionality
                    value={selectedCategories} // Bind the selected values to state
                    onChange={handleCategoryChange} // Update state when selection changes
